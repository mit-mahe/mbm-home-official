document.addEventListener("DOMContentLoaded", () => {
	...
	const basePath = "breakingbond_anim/00";
	const numberOfImages = 28; //change depending on the video
	const imagePaths = [];

	const renderingWidth = 100;
	const renderingHeight = 100;

	const lastFWidth = 41.667;
	const lastFHeight = 74.074;
	const lastFXoffset = 31.25;
	const lastFYoffset = 8.333;
	...
	const lastFWidthRES = vwToPx(lastFWidth);
	const lastFHeightRES = vhToPx(lastFHeight);

	const lastFXoffsetRES = vwToPx(lastFXoffset);
	const lastFYoffsetRES = vhToPx(lastFYoffset);
	...
	function renderFrame(frameIndex) {
		...
		if (frameIndex === 27) {
			...
			img.onload = function () {
				...
				// Draw the image on the canvas with specified dimensions
				context.drawImage(img, 0, 0, renderingWidthRES, renderingHeightRES);
			};
		}
			...
			scrollTrigger: {
				...
				onUpdate: function (self) {
					...
					if (frameIndex === 27) {
						...
						canvas.classList.add("BBresized");
						...
						outerDiv.style.backgroundColor = "#344546";
					} else {
						canvas.classList.remove("BBresized");
						...
						outerDiv.style.backgroundColor = "#608488";
					}
				},
			},
		}
