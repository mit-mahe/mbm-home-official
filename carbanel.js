document.addEventListener("DOMContentLoaded", () => {
	...
	const basePath = "carbanel_anim/png_0";
	const numberOfImages = 61; //change depending on the video
	const imagePaths = [];

	imagePaths.push("carbanel_anim/png_0001.png");
	imagePaths.push("carbanel_anim/png_0001.png");
	imagePaths.push("carbanel_anim/png_0001.png");

	const renderingWidth = 83.333;
	const renderingHeight = 125;
	const fromLeft = 7.5;
	...
	const fromLeftRES = vwToPx(fromLeft);
		...
		{},
		{
			...
			scrollTrigger: {
				...
				onUpdate: function (self) {
					...
					const frameIndex = Math.round(progress * (64 - 1)); // select the frame based on that
					renderFrame(frameIndex);
					if (frameIndex === numberOfImages + 2) {
						// numberOfImages - 1 + 3, because of the 3 extra starting frames
